...
int main(void) {
    ...
    printf("El exponente vale: %d \n", var.EXPO);  // uso %u a %d para imprimir un entero con signo
    printf("La mantiza es: %u \n", var.MANTIZA);
    
    printf("El número segun IEEE 754 es: ");
    
    for (int i = 3; i >= 0; i--) {
        if (var.byte[i] < 0x10) {  // Si el primer carácter del hexadecimal es 0, normalmente no lo imprime
            printf("0");
        }
        printf("%X", var.byte[i]);
    }
