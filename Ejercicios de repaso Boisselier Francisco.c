...
int main(void){
    ...
    printf("\n--- Menu ---\n1. Ingresar datos de empleado \n2. Procesar y mostrar informacion  \n3. Salir \nElija una de las opciones: ");
    scanf("%d", &opciones);


    while(opciones !=3){
        while(opciones !=1 && opciones !=2){
            printf("\nLa opcion ingresada no se encuentra en el menu, las opciones disponibles son 1, 2 y 3");
            printf("\nVuelva a ingresar: ");
            scanf("%d", &opciones);
        }
        if(opciones==1){
            ...
            printf("\nEmpleado ingresado. Actualmente hay %d de 5 empleados ingresados.\n", empleadosIngresados);
            printf("\n--- Menu ---\n1. Ingresar datos de empleado \n2. Procesar y mostrar informacion  \n3. Salir \nElija una de las opciones: ");
            scanf("%d", &opciones);
        }
        if(opciones==2){
...
void procesarDatos(struct Empleado empleados[], uint8_t empleadosIngresados){
    ...
    printf("\nEl salario promedio es de $%f", promedio);
    printf("\nEl empleado con mejor salario es %s con un salario de: $%d\n", empleados[maxEmpleado].nombre, max);
